...
game.Player.prototype.update = function(deltaTime) {
  ...
  if (Game.controler.LEFT) {
    this.x -= Math.cos(this.angle - Math.PI / 2) * Game.controler.DOWN * this.velocity * deltaTime / 1000;
    this.y -= Math.sin(this.angle - Math.PI / 2) * Game.controler.DOWN * this.velocity * deltaTime / 1000;
    console.log(this.x, this.y, this.angle, this.angle - Math.PI/2);
    //this.x -= Math.cos(this.angle) * Game.controler.LEFT * this.velocity * deltaTime / 1000;
  }
  if (Game.controler.RIGHT) {
    this.x += Math.cos(this.angle + Math.PI / 2) * Game.controler.DOWN * this.velocity * deltaTime / 1000;
    this.y += Math.sin(this.angle + Math.PI / 2) * Game.controler.DOWN * this.velocity * deltaTime / 1000;
    console.log(this.x, this.y, this.angle, this.angle + Math.PI/2);
    //this.x += Math.cos(this.angle) * Game.controler.RIGHT * this.velocity * deltaTime / 1000;
  }
