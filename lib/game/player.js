...
game.Player = function(params) {
  ...
  this.id = params.id;
  this.isNaked = params.isNaked == undefined ? params.isNaked : true;
  this.angle = 0;
...
game.Player.prototype.updateVision = function(deltaTime) {
  for (var i = 0; i < Game.playerManager.objs.length; i++) {
    ...
    if (this.isInFOV(player.x, player.y)) {
      console.log('IN FOV', player.state, player.isNaked);
      var inHisFOV = player.isInFOV(this.x, this.y);

      // Deux joueurs nue se voie mutuellement
      if (inHisFOV && this.isNaked && player.isNaked) {
        this.inFOVBothNaked(deltaTime);
      }
      // Dans son FOV, Moi nue, Lui habille
      else if (inHisFOV && this.isNaked && !player.isNaked) {
        this.inFOVMeNaked(deltaTime);
      }
      // Dans son FOV, Moi habille, Lui nue
      else if (inHisFOV && !this.isNaked && player.isNaked) {
        this.inFOVHesNaked(deltaTime);
      }
      // Dans son FOV, Moi habille, Lui Habille
      else if (inHisFOV && !this.isNaked && !player.isNaked) {
        this.inFOVBothDressed(deltaTime);
      }
      // Dans son FOV, Moi avec ses slips, Lui avec mes slips
      else if (false) {
      }
      // Dans son FOV, Moi avec ses slips, Lui nue
      else if (false) {
      }
      // Dans son FOV, Moi avec ses slips, Lui habille
      else if (false) {
      }
      // Dans son FOV, Moi habille, Lui avec mes slips
      else if (false) {
      }
      // Dans son FOV, Moi nue, Lui avec mes slips
      else if (false) {
      }
      // Il ne me voie pas...
      // Moi nue, Lui nue
      else if (!inHisFOV && this.isNaked && player.isNaked) {
        this.notInFOVBothNaked(deltaTime);
      }
      // Moi nue, Lui habille
      else if (!inHisFOV && this.isNaked && !player.isNaked) {
        this.notInFOVMeNaked(deltaTime);
      }
      // Moi habille, Lui nue
      else if (!inHisFOV && !this.isNaked && player.isNaked) {
        this.notInFOVHesNaked(deltaTime);
      }
      // Moi habille, Lui habille
      else if (!inHisFOV && !this.isNaked && !player.isNaked) {
        this.notInFOVBothDressed(deltaTime);
      }
      // Moi avec ses slips, Lui avec mes slips
      else if (false) {
      }
      // Moi avec ses slips, Lui nue
      else if (false) {
      }
      // Moi avec ses slips, Lui habille
      else if (false) {
      }
      // Moi habille, Lui avec mes slips
      else if (false) {
      }
      // Moi nue, Lui avec mes slips
      else if (false) {
      }
    }
  }
...
/** */
game.Player.prototype.inFOVBothNaked = function(deltaTime) {
};


/** */
game.Player.prototype.inFOVMeNaked = function(deltaTime) {
};


/** */
game.Player.prototype.inFOVHesNaked = function(deltaTime) {
};


/** */
game.Player.prototype.inFOVBothDressed = function(deltaTime) {
};


/** */
game.Player.prototype.notInFOVBothNaked = function(deltaTime) {
};


/** */
game.Player.prototype.notInFOVMeNaked = function(deltaTime) {
};


/** */
game.Player.prototype.notInFOVHesNaked = function(deltaTime) {
};


/** */
game.Player.prototype.notInFOVBothDressed = function(deltaTime) {
};
