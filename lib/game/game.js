...
GameEngine.prototype.updateGame = function(deltaTime){
  ...
  //console.log(this.player.shamePoints, this.player.disgustPoints);
};
...
GameEngine.prototype.updateInitgame = function(deltaTime){
  if(this.isNewState){
    ...
    socket.once('game-init', function(data){
        ...
        Game.map = new JSGame.World('4fdcb3c944beb60100000004', net);
...
GameEngine.prototype.renderGame = function(){
  ...
  var mapX = 0, mapY = 0;
      if(this.player) {
        mapX = this.player.x/JSGame.tilesSize;
        mapY = this.player.y/JSGame.tilesSize;
      }
